...
export class SizeService {
    ...
    async update(id: string, changeDto: createSizeDto) {
        ...
        if (product) {
            ...
            product.product = await inventory.map((valorImage) => this.productImageRepository.create({ brands: valorImage }))
        } else {
            product.product = await this.productImageRepository.findBy({ product: { id } });
        }
